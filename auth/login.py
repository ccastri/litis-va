...
redis_client =  redis.StrictRedis(host='redis', port=6379, password='63709598', decode_responses=True)
# oauth = OAuth(
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    try:
        ...
        manual_token = os.getenv("NC_JWT_TOKEN")
        set_redis_jwt(user.email, manual_token)
        set_redis_jwt(user.username, access_token)

        print(f"Storing tokens in Redis: {user.email} -> {manual_token}, {user.username} -> {access_token}")        
        response_headers = {
...
def set_redis_jwt(key: str, token: str):
    try:
        redis_client.set(key, token)
        print(f"Set token in Redis: {key} -> {token}")
    except Exception as e:
        print(f"Error al configurar el token en Redis: {e}")
   


@router.get("/api/get-token/{email}")
